...
with(imports) {
	...
	function setDescriptionLines(player, descriptionLines) {
		var lines = [
			...
			"what he was about to do. This can be",
			"helped by setting other players to his side."
		];
	...
	var FAILD_CLUMSY = "THE GORILLA WAS TOO CLUMSY AND SQUEEZED THE TEAMMATE TO DEATH WITH HIS BARE HANDS";
    	...
    	var ruleMoveStupid = new RuleMoveStupid(rulesArray[0].getActor()){
    		apply: function(message, path){
    			...
    			if(player.getSpecialStat(stupidKey) == "true"){
    				_super_.sendMessageShowMe(player.getTeam().getCoach(), player.toString(), stupidString);
    			}else{
    			...
    			if(player.getSpecialStat(stupidKey) == "true"){
	    			...
	    			if(player.invokeGetRemainingBe() > 0){
	        			...
	        			if(playerCounter > 0){
	        				...
	        				_super_.sendMessageShowMe(player.getTeam().getCoach(), player.toString(), "I can't find any throwable player!");
